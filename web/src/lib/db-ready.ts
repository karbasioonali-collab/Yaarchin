...
const schemaState = cache(async (): Promise<{ catalog: boolean; importScore: boolean; customer: boolean }> => {
  const r = await db.execute<{ catalog: boolean; import_score: boolean; customer: boolean }>(sql`
    select
